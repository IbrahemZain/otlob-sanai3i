...
class DownCard extends StatelessWidget {
  ...
  DownCard(this.index, this.san3a, this.allData);
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {
        ...
        // netWorkHelper.increaseMostView(allData[index]['technician']['_id']);
        Navigator.push(context,
            MaterialPageRoute(builder: ((context) => ProfilePage(isUser: false,index: index,mvUsers: allData,))));
        return;
      },
      child: Container(
          margin: EdgeInsets.symmetric(horizontal: _deviceWidth*.01,vertical: _deviceHeight*.01),
          ...
          child: Stack(
            children: <Widget>[
              Positioned(
                ...
                right: _deviceWidth*.04,
                child: Text(
                  allData[index]['technician']['name'],
                  style: TextStyle(color: Colors.white, fontSize: 12.5),
                ),
              ),Positioned(
                top: _deviceHeight * .040,
                right: _deviceWidth*.03,
